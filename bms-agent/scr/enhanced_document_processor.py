...
class ContextualRetrievalEngine:
    ...
    def generate_chunk_context(self,
        ...
        Returns: Dict with 'content' and 'context_metadata' keys
        """
        ...
        # Store context as structured metadata instead of prepending to content
        context_metadata = {
            'document_title': doc_title,
            'document_type': doc_type,
            'document_summary': doc_summary[:200] if doc_summary else None,
            'chunk_position': chunk_index + 1,
            'total_chunks': total_chunks,
            'chunk_summary': chunk_summary,
            'preceding_topics': self._extract_key_topics(prev_chunk) if prev_chunk else None,
            'following_topics': self._extract_key_topics(next_chunk) if next_chunk else None
        }

        # Generate context string for backwards compatibility (can be used in embeddings if needed)
        context_parts = [
            f"Document: {doc_title} ({doc_type})",
            f"Section {chunk_index + 1} of {total_chunks}",
            f"Content: {chunk_summary}"
        ]
        if doc_summary:
            context_parts.insert(1, f"Summary: {doc_summary[:200]}")
        if prev_chunk:
            context_parts.append(f"Follows: {context_metadata['preceding_topics']}")
        if next_chunk:
            context_parts.append(f"Precedes: {context_metadata['following_topics']}")

        context_string = " | ".join(context_parts)

        # Return content unchanged, with context as metadata
        return {
            'content': chunk,  # Original content, no bloat
            'context_metadata': context_metadata,
            'context_string': context_string  # For optional use in embeddings
        }
...
class HierarchicalChunkingEngine:
    ...
    def _create_child_chunks(self, parent_content: str) -> List[Dict]:
        """Create child chunks from parent content, respecting sentence boundaries"""
        child_size = self.config.child_chunk_size
        ...
        if NLTK_AVAILABLE:
            ...
            for sent in sentences:
                ...
                # If adding this sentence exceeds size and we have content, finalize chunk
                if current_size + sent_len > child_size and current_chunk:
                    chunk_content = ' '.join(current_chunk)
                    ...
                    position += current_size - overlap_size
                    current_chunk = overlap_sentences + [sent]
                    current_size = overlap_size + sent_len
                else:
                    current_chunk.append(sent)
            ...
            # Fallback to character-based chunking if NLTK not available
...
class QualityValidationEngine:
    ...
    def __init__(self, config: ProcessingConfig):
        ...
        # Quality thresholds - LOWERED from strict research-grade (95%+) to practical production levels
        # These thresholds are now advisory; primary validation uses overall_score
        self.thresholds = {
            'faithfulness': float(os.getenv('BMS_QUALITY_FAITHFULNESS', '0.60')),     # Was 0.95
            'answer_relevancy': float(os.getenv('BMS_QUALITY_RELEVANCY', '0.55')),   # Was 0.90
            'context_precision': float(os.getenv('BMS_QUALITY_PRECISION', '0.55')),  # Was 0.85
            'context_recall': float(os.getenv('BMS_QUALITY_RECALL', '0.50')),        # Was 0.80
            'semantic_similarity': float(os.getenv('BMS_QUALITY_SIMILARITY', '0.50')) # Was 0.75
        }

        # Validation mode: 'strict' (all thresholds) or 'balanced' (overall_score only)
    ...
    def validate_chunk_quality(self, 
        ...
        # Mode 'balanced' (default): Use overall_score only, individual thresholds are advisory
        # Mode 'strict': All individual thresholds must pass (original behavior)
        if self.validation_mode == 'strict':
            ...
            # Use overall_score with config threshold (default 70%)
            passes_quality = overall_score >= (self.config.min_quality_score / 100.0)

        return {
            'metrics': metrics,
            'overall_score': overall_score * 100,  # Convert to percentage for display
...
class EnhancedDocumentProcessor:
        ...
        try:
            ...
            if self.config.enable_quality_validation:
                ...
                for chunk in chunks:
                    ...
                    # Quality validation now uses 'balanced' mode by default (overall_score only)
                    # Pass if quality validation passes (which checks overall_score in balanced mode)
                    if quality['passes_quality']:
                        ...
                        # Log why chunk failed for debugging
                        logger.debug(f"Chunk failed quality: score={quality['overall_score']:.1f}%, "
                                   f"mode={quality['validation_mode']}, "
                                   f"failed_metrics={quality['failed_metrics']}")
    ...
    def _read_document(self, file_path: Path) -> str:
        ...
        if extension in ['.txt', '.md']:
            ...
            try:
                ...
                logger.warning(f"Error reading DOCX file (returning partial content): {e}")
                # Return whatever we extracted so far instead of failing completely
                return '\n'.join(text) if text else ''

        elif extension in ['.pptx', '.ppt']:
            # Only .pptx is supported by python-pptx, not old .ppt format
            if extension == '.ppt':
                logger.warning(f"Old .ppt format not supported for {file_path}, returning empty content")
                return ''
            try:
                ...
                logger.warning(f"Error reading PPTX file (returning empty): {e}")
                return ''
            ...
            if PANDAS_AVAILABLE:
                try:
                    ...
                    logger.warning(f"Error reading Excel file (returning empty): {e}")
                    return ''
            else:
    ...
    def _apply_contextual_retrieval(self,
        ...
        Stores context as metadata instead of modifying content
        """
