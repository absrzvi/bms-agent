...
- **Performance**: Standard documents (≤10 sections, ≤100 table rows, ≤6 slides) must generate within 10 seconds.
- **Reliability**: Restart workflow must cleanly stop previous process, start a new one, and update PID/log files.
- **Security**: Pipeline remains bound to localhost; default API key sufficient for POC, to be hardened later.
- **Maintainability**: Scripts and documentation must allow redeployment without manual edits and follow existing style.
- **Storage**: Ensure output directory exists with 755 permissions; document manual cleanup practices.

## 6. Architecture & Integration
- **Topology**: OpenWebUI and Pipelines server share the RunPod environment. Pipelines server runs from cloned repository (`/tmp/pipelines-repo`), while configuration and outputs reside in `pipelines/`.
- **Data Flow**: User prompt → OpenWebUI LLM → Function call to Pipelines server → Library generates document → File path returned to user for download.
- **Logging**: `nohup` output routed to `pipelines/server.log`; errors surfaced in test script output.
...
- **Backups**: Encourage optional copy to `/workspace/001-bms-agent/generated-documents/`; automated retention out of scope.
